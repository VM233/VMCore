...
{
    public static class VisualElementQueryUtility
    {
        #region Get Children
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<VisualElement> QueryAll(this VisualElement root)
        {
            return root.PreorderTraverse(true, element => element.Children());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<VisualElement> QueryAll(this IEnumerable<VisualElement> roots)
        {
            foreach (var root in roots)
            {
                foreach (var element in root.QueryAll())
                {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<T> QueryAll<T>(this VisualElement root) where T : VisualElement
        {
            return root.PreorderTraverse<VisualElement, T>(true, element => element.Children());
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<T> QueryAll<T>(this IEnumerable<VisualElement> roots) where T : VisualElement
        {
            foreach (var root in roots)
            {
                foreach (var element in root.QueryAll<T>())
                {
                    yield return element;
                }
            }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<T> QueryAll<T>(this VisualTreeAsset treeAsset) where T : VisualElement
        {
            if (treeAsset == null)
            {
            ...
            return treeAsset.CloneTree().contentContainer.QueryAll<T>();
        }
        ...
        #region Query All By Name

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static IEnumerable<TVisualElement> QueryAllByName<TVisualElement>(this VisualElement root, string name)
            where TVisualElement : VisualElement
        {
            foreach (var element in root.QueryAll<TVisualElement>())
            {
        ...
        {
            return root.QueryAll<VisualElement>().Select(visualElement => visualElement.name);
        }
        ...
        public static IEnumerable<string> GetAllNames(this VisualTreeAsset treeAsset)
        {
            ...
            return treeAsset.QueryAll<VisualElement>()
        ...
        public static VisualElement QueryStrictly(this VisualElement root, string name, string nameSourceName)
        {
            ...
            visualElement.AssertIsNotNull($"{nameSourceName} of {nameof(VisualElement)}");
            ...
            where T : VisualElement
        {
            ...
            visualElement.AssertIsNotNull($"{nameSourceName} of {typeof(T)}");
        ...
        public static IEnumerable<VisualElement> Query(this VisualElement root, IEnumerable<string> names)
        {
            ...
            foreach (var visualElement in root.QueryAll())
            {
